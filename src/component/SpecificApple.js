...
import useSpecificApple from "../utils/useSpecificApple";

const SpecificApple = () => {
  const {id11} = useParams();
  const[item,error,loading] = useSpecificApple(id11);
  const { seller } = useContext(UserContext);
  const dispatch = useDispatch();
  const [items] = useApple()
      ...
      <div className="specific-item-container trending-container">
        ...
        <div className="right-section">
          ...
          <div className="action-buttons">
            <button onClick={() => addApple(item)} className="add-to-cart-btn">
              Add to Cart
            </button>
