...
import SamsungData from './SamsungData'
import useSamsung from '../utils/useSamsung';
import useSpecificSamsung from '../utils/useSpecificSamsung';

const SpecificSamsung = () => {
  const {id10} = useParams();
  const[item,error,loading] = useSpecificSamsung(id10);
  const { seller } = useContext(UserContext);
  const dispatch = useDispatch();
  const [items] = useSamsung()

    if (loading === true) return <Shimmer />;
      if (error) return <p>Error: {error}</p>;
      
        const addSamsung = (item) => {
      ...
      <div className="specific-item-container trending-container">
        ...
        <div className="right-section">
          ...
          <div className="action-buttons">
            <button onClick={() => addSamsung(item)} className="add-to-cart-btn">
              Add to Cart
            </button>
...
export default SpecificSamsung
