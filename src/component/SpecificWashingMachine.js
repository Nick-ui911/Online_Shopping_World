...
import useSpecificWashingMachine from "../utils/useSpecificWashingMachine";
import WashingMachineData from "./WashingMachineData";
import useWashingMachine from "../utils/useWashingMachine";


const SpecificWashingMachine = () => {

  const {id8} = useParams();
  const[item,error,loading] = useSpecificWashingMachine(id8);
  const { seller } = useContext(UserContext);
  const dispatch = useDispatch();
  const [items] = useWashingMachine()

   if (loading === true) return <Shimmer />;
    if (error) return <p>Error: {error}</p>;
    
      const addWashingMachine = (item) => {
      ...
      <div className="specific-item-container trending-container">
        ...
        <div className="right-section">
          ...
          <div className="action-buttons">
            <button onClick={() => addWashingMachine(item)} className="add-to-cart-btn">
              Add to Cart
            </button>
...
export default SpecificWashingMachine
