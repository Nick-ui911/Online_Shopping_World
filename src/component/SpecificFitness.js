...
import useFitness from "../utils/useFitness";
import useSpecificFitness from "../utils/useSpecificFitness";

const SpecificFitness = () => {
  
  const {id22} = useParams();
  const[item,error,loading] = useSpecificFitness(id22);
  const { seller } = useContext(UserContext);
  const dispatch = useDispatch();
  const [items] = useFitness()

   if (loading === true) return <Shimmer />;
    if (error) return <p>Error: {error}</p>;
    
      const addFitness = (item) => {
      ...
      <div className="specific-item-container trending-container">
        ...
        <div className="right-section">
          ...
          <div className="action-buttons">
            <button onClick={() => addFitness(item)} className="add-to-cart-btn">
              Add to Cart
            </button>
...
export default SpecificFitness
