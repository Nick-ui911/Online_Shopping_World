...
import useSpecificSkinCare from "../utils/useSpecificSkinCare";
import useSkinCare from "../utils/useSkinCare";
import SkinCard from "./SkinCard";

const SpecificSkinCare = () => {
  const { id6 } = useParams();
  const [item, isLoading, error] = useSpecificSkinCare(id6)
  const { seller } = useContext(UserContext);
  const dispatch = useDispatch();
  const[items]=useSkinCare();
  if (isLoading === true) return <Shimmer />;
  if (error) return <p>Error: {error}</p>;

  const addSkinItem = (item) => {
      ...
      <div className="specific-item-container trending-container ">
        ...
        <div className="right-section">
          ...
          <div className="action-buttons">
            <button onClick={() => addSkinItem(item)} className="add-to-cart-btn">
              Add to Cart
            </button>
...
export default SpecificSkinCare;
