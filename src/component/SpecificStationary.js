...
import useStationary from "../utils/useStationary";
import useSpecificStationary from "../utils/useSpecificStationary";

const SpecificStationary = () => {
  
  const {id20} = useParams();
  const[item,error,loading] = useSpecificStationary(id20);
  const { seller } = useContext(UserContext);
  const dispatch = useDispatch();
  const [items] = useStationary()

   if (loading === true) return <Shimmer />;
    if (error) return <p>Error: {error}</p>;
    
      const addStationary = (item) => {
      ...
      <div className="specific-item-container trending-container">
        ...
        <div className="right-section">
          ...
          <div className="action-buttons">
            <button onClick={() => addStationary(item)} className="add-to-cart-btn">
              Add to Cart
            </button>
...
export default SpecificStationary
