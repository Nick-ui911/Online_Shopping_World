...
import HomeKitchenData from './HomeKitchenData';
import useHomeKitchen from '../utils/useHomeKitchen';
import useSpecificHomeKitchen from '../utils/useSpecificHomeKitchen';

const SpecificHomeKitchen = () => {

  const {id21} = useParams();
  const[item,error,loading] = useSpecificHomeKitchen(id21);
  const { seller } = useContext(UserContext);
  const dispatch = useDispatch();
  const [items] = useHomeKitchen()

   if (loading === true) return <Shimmer />;
    if (error) return <p>Error: {error}</p>;
    
      const addHomekitchen = (item) => {
      ...
      <div className="specific-item-container trending-container">
        ...
        <div className="right-section">
          ...
          <div className="action-buttons">
            <button onClick={() => addHomekitchen(item)} className="add-to-cart-btn">
              Add to Cart
            </button>
...
export default SpecificHomeKitchen
