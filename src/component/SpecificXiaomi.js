...
import useSpecificXiaomi from '../utils/useSpecificXiaomi';
import XiaomiData from "./XiaomiData";
import useXiaomi from '../utils/useXaomi';

const SpecificXiaomi = () => {
  const {id13} = useParams();
  const[item,error,loading] = useSpecificXiaomi(id13);
  const { seller } = useContext(UserContext);
  const dispatch = useDispatch();
  const [items] = useXiaomi()

    if (loading === true) return <Shimmer />;
      if (error) return <p>Error: {error}</p>;
      
        const addXiaomi = (item) => {
    ...
    <div className="specific-item-container trending-container">
      ...
      <div className="right-section">
        ...
        <div className="action-buttons">
          <button onClick={() => addXiaomi(item)} className="add-to-cart-btn">
            Add to Cart
          </button>
