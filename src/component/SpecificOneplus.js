...
import useOneplus from '../utils/useOneplus';
...
import useSpecificOneplus from '../utils/useSpecificOneplus';

const SpecificOneplus = () => {
  const {id12} = useParams();
  const[item,error,loading] = useSpecificOneplus(id12);
  const { seller } = useContext(UserContext);
  const dispatch = useDispatch();
  const [items] = useOneplus()

    if (loading === true) return <Shimmer />;
      if (error) return <p>Error: {error}</p>;
      
        const addOneplus = (item) => {
      ...
      <div className="specific-item-container trending-container">
        ...
        <div className="right-section">
          ...
          <div className="action-buttons">
            <button onClick={() => addOneplus(item)} className="add-to-cart-btn">
              Add to Cart
            </button>
...
export default SpecificOneplus
